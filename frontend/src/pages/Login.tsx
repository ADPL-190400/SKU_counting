...
export function Login({ onLogin }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setError(err instanceof ApiRequestError ? err.message : t("login.connect_error"));
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="min-h-screen flex items-center justify-center bg-bg p-4">
      <form onSubmit={handleSubmit} className="glass-panel w-full max-w-sm p-8">
        <div className="flex flex-col items-center mb-6">
          ...
          <div className="text-sm font-bold tracking-widest text-text">{t("header.title")}</div>
          <div className="text-xs text-text-faint mt-1">{t("login.subtitle")}</div>
        </div>

        <div className="space-y-4">
          <div>
            <label className="block text-xs text-text-dim mb-1">{t("login.email_label")}</label>
            <input
              autoFocus
              type="email"
          ...
          <div>
            <label className="block text-xs text-text-dim mb-1">{t("login.password_label")}</label>
            <input
              type="password"
